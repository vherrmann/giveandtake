...
signupHandler signupData = do
  ...
  startVerifEmailjob userId signupData
 where
...
startVerifEmailjob :: (HasHandler m) => UserId -> SignupData -> m JobId
...
checkHandler (SA.Authenticated user) = pure CheckResponse{user = P.entityVal user, userId = user.key}
